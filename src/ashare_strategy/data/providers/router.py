...
def build_data_provider(config: StrategyConfig) -> MarketDataProvider:
    ...
    if provider_name == 'akshare':
        return AkshareProvider(cache_dir=config.data_cache_dir, use_cache=config.use_cache, timeout_seconds=config.data_source.timeout_seconds, max_retries=config.data_source.max_retries, offline_mode=config.data_source.offline_mode)
