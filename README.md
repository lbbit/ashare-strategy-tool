...
> 不要求你会编程，不要求你懂量化，只要会按步骤点命令，就能用起来。
...
简单理解：
> 它不是替你自动下单，而是把你每天手工做的“翻股票、记持仓、做复盘”流程尽量自动化。
...
## 最推荐的新手使用路径
如果你是第一次用，建议按下面顺序：
1. 先运行 `screen` 看看候选股输出长什么样
2. 再运行 `save-sample-positions` 生成一个示例持仓
3. 接着运行 `positions` 看持仓格式
4. 然后运行 `plan` 生成每日计划
5. 最后运行 `backtest` 看历史效果
6. 如果你不喜欢命令行，再运行 `ui` 打开图形界面
...
## Star 趋势
如果仓库开始积累更多用户，可以在 README 中展示 Star 趋势图。当前已预留常用写法：
